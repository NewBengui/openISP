...
@app.route('/')
def index():
    files_data, errors = sftp_utils.fetch_sftp_files()
    if not files_data:
        return render_template('index.html', error="Nenhum dado encontrado ou ocorreu um erro.", errors=errors)
    return render_template('index.html', files_data=files_data, errors=errors)

@app.route('/api/data')
def get_data():
    files_data, errors = sftp_utils.fetch_sftp_files()
    if not files_data:
        return jsonify({"error": "Nenhum dado encontrado ou ocorreu um erro.", "details": errors}), 500
